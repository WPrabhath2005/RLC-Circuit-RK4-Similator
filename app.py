...
R_input=st.sidebar.number_input('◾ Resistance (Ohms)',value=100.0)
L_input=st.sidebar.number_input('◾ Inductance (mH)',value=1.0)
...
if st.session_state.simulated:
    ...
    if V_input=='AC':
        ...
        # Calculate data points per cycle
        h_ms=t_list[1]-t_list[0]
        points_per_cycle=int(T_ms/h_ms)

        # Extract the last cycle to prevent cycle jumping
        ...
        # Find peak values in the last cycle only
    ...
    # Create an empty placeholder for the graph frame
    graph_screen=st.empty()

    # Calculate fixed Y-axis limits to prevent screen shaking during animation
    v_max=max(vc_list)
    v_min=min(vc_list)
    i_max=max(i_list)
    i_min=min(i_list)
    ...
    if st.session_state.animate:
        steps=20
        chunk_size=len(t_list)//steps

        for step in range(1,steps+1):
            end_idx=step*chunk_size
            if step==steps: end_idx=len(t_list)
            ...
            ax1.plot(t_list[:end_idx],vc_list[:end_idx],color='blue',linewidth=1.5,label='Voltage(V)')
            ...
            ax2.plot(t_list[:end_idx],i_list[:end_idx],color='red',linewidth=1.5,label='Current(mA)')
            ...
            graph_screen.pyplot(fig)
            plt.close(fig)
            time.sleep(0.05)

        st.session_state.animate=False

    # Final Static Graph (Locks the graph on screen after the animation completes)
    fig,ax1=plt.subplots(figsize=(10,6))
    ax1.set_xlim(0,t_list[-1])
    ax1.set_ylim(v_min-v_pad,v_max+v_pad)
